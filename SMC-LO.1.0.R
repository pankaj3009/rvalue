...
kBackTestEndDate = strftime(adjust("India", as.Date(kBackTestEndDate, tz = kTimeZone), bdc =
                                           2), "%Y-%m-%d")
kBackTestStartDate = strftime(adjust("India", as.Date(kBackTestStartDate, tz = kTimeZone), bdc =
                                             0), "%Y-%m-%d")
kBackTestCloseAllDate = strftime(adjust("India", as.Date(kBackTestCloseAllDate, tz = kTimeZone), bdc =
                                                0), "%Y-%m-%d")
                ...
                for (l in 1:nrow(portfolio)) {
                        ...
                        if (is.na(portfolio[l, 'exittime'])) {
                                ...
                                if (length(price) == 1) {
                                        ...
                                        buyindex = which(
                                                as.Date(md$date, tz = "Asia/Kolkata") == portfolio[l, 'entrytime']
                                        )
                                        mtmindex = which(
                                        ...
                                        unrealizedprofit = unrealizedprofit + portfolio[l, 'size'] * splitadjustment * portfolio[l, 'mtm'] - portfolio[l, 'size'] *
                                                portfolio[l, 'entryprice'] - kBrokerage * portfolio[l, 'size'] * splitadjustment * portfolio[l, 'mtm'] - kBrokerage * portfolio[l, 'size'] * portfolio[l, 'entryprice']
                                } else{
                                        npv = npv + ifelse(is.na(portfolio[l, 'mv']),
                                                           0,
                                                           portfolio[l, 'mv'])
                                }
                        } else{
...
UpdateDF4Upside <-
        function(df4,
                ...
                a <-
                        unlist(redisSMembers("symbolchange")) # get values from redis in a vector
                tmp <-
                        (strsplit(a, split = "_")) # convert vector to list
                k <-
                        lengths(tmp) # expansion size for each list element
                ...
                symbolchange$date = as.POSIXct(symbolchange$date, format = "%Y%m%d", tz =
                                                       "Asia/Kolkata")
                ...
                for (i in 1:nrow(df4)) {
                        symbol = df4[i, 'TICKER']
                        if (file.exists(paste(datafolder,
                                              symbol,
                                              ".Rdata",
                                              sep = ""))) {
                                load(paste(datafolder,
                                           symbol,
                                           ".Rdata",
                                           sep = ""))
                                ...
                                if (length(endlength) == 1 &&
                                    endlength > (WorkingDaysForSlope - 1)) {
                                        ...
                                        startlength = endlength - (WorkingDaysForSlope -
                                                                           1)
                                        ...
                                        shareoutstandingindex = tail(which(
                                        ...
                                        if (length(shareoutstandingindex) == 1) {
                                                ...
                                                df4$MCAP[i] = (
                                                        as.numeric(
                                                                df4$COMMON_SHARE_OUTSTANDING_IN_MILLIONS[i]
                                                        ) * md[shareoutstandingindex, 'splitadjust'] / md[endlength, 'splitadjust']
                                                ) *
                                                        lastprice  / 1000
                                                df4$SplitOverLastYear[i] = md[startlength, 'splitadjust'] /
                                                        md[endlength, 'splitadjust']
                                        }
                                } else{
                                        # data does not exist for a year...
                                        df4$UPSIDE[i] = -10000
                                        df4$SplitOverLastYear[i] = 0
                                }
                        }
                }
                df4
